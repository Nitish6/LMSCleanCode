...
public class HorrorBookImpl implements BookCategory {
	...
	public Book getBookCategory(String category) {
		...
		List<Book> horrorBook = dataList.getHorrorBooks();
		horrorBook.forEach(bk -> {
			System.out.println("book name : " + bk.getName() + "  book Id: " + bk.getBid());
		});
		System.out.println();
		System.out.println("Enter Book Id: ");
		int id = sc.nextInt();
		bookSearched = horrorBook.stream().filter((bk) -> bk.getBid() == id).findAny().orElse(null);
